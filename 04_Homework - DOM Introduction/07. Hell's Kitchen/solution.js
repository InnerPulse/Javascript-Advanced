    ...
    function onClick() {
        ...
        // Assigning highest and average salaries
         for (const r in allRestaurants) {
          allRestaurants[r].highestSalary = Number(highestSalary(allRestaurants[r].workers));
          allRestaurants[r].averageSalary = Number(averageSalary(allRestaurants[r].workers));

         //  Find best restaurant by mentioned criteria
         if (bestRestaurant) {
        ...
        // Parse data to workers and their slaries
        ...
        // Find highest salary
        function highestSalary(workers) {
            return workers
                .reduce((acc, c) => (c.salary > acc ? c.salary : acc), 0)
                .toFixed(2);
        }
        ...
        function averageSalary(workers) {
            return (
                workers.reduce((acc, c) => acc + c.salary, null) /
                workers.length
            ).toFixed(2);
        }
